...
# ── 5. 로컬 다운로드 안내 ──
APP_PUBLIC_IP=$(curl -sf http://169.254.169.254/latest/meta-data/public-ipv4 2>/dev/null || echo "<APP_PUBLIC_IP>")
...
echo "  === 로컬로 다운로드 ==="
echo ""
echo "  scp -i ~/.ssh/onlyone-loadtest.pem -r \\"
echo "    ubuntu@${APP_PUBLIC_IP}:~/OnlyOne-Back/results/ \\"
