...
class ImageControllerTest {
    ...
    void presign_success_returns_upload_and_cloudfront_view_url() throws Exception {
        ...
        // when & then
        mockMvc.perform(post("/{imageFolderType}/presigned-url", "CHAT")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body)
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.presignedUrl").value(resp.getPresignedUrl()))
                .andExpect(jsonPath("$.data.imageUrl").value(resp.getImageUrl()));
    }
