...
public class SseConfig {
    ...
    /**
     * 알림 처리용 Virtual Thread Executor (동시실행 제한)
     * 배치 처리 시 과도한 스레드 생성 방지
     */
    @Bean("notificationExecutor")
    public Executor notificationExecutor(
            @Value("${app.notification.notification-executor-permits:100}") int permits) {
        ThreadFactory tf = Thread.ofVirtual().name("notification-", 0).factory();
        ExecutorService delegate = Executors.newThreadPerTaskExecutor(tf);
        log.info("Notification Bounded Virtual Thread Executor 설정: permits={}", permits);
        return new AsyncConfig.BoundedVtExecutor(delegate, permits, 30);
    }
}